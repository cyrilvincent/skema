...
import delimited "C:\Users\benjamin.montmartin\Desktop\Etude UFC 2023\Part III\psychiatres.csv", clear
import delimited "C:\Users\conta\git-CVC\Skema\Part III\psychiatres.csv", clear
rename genre gender
...
keep if strpos(codeccamdelacte, "CNP")
tab codeccamdelacte

///Tarif S1: 50.2 euros (42.5 (CS) +2.7 (MPC) +5 (MCS)) 


distinct ps_id
**2342 praticiens font des consultations soit la totalité****
...
keep if codeccamdelacte=="CNP" |codeccamdelacte=="CNP+MPC+MCS"

replace mp=50.2 if convention==1 & optioncontratdaccèsauxsoins=="false"
...
replace prixmoyen=50.2 if prixmoyen==0
...
gen exessr=((prixmoyen-50.2)/50.2)*100
...
by dep: gen depmoyen=[(PrixMoyen-50.2)/50.2]*100
drop PrixMoyen
rename Prix PrixMoyen
gen depmoyen_F=[(PF-50.2)/50.2]*100
...
by dep: gen depmoyenS2=[(PrixMoyenS2-50.2)/50.2]*100
by dep: egen dpS2=mean(depmoyenS2)
drop PrixMoyenS2
rename PrixS2 PrixMoyenS2
drop depmoyenS2
rename dpS2 depmoyenS2
gen depmoyen_FS2=[(PFS2-50.2)/50.2]*100
