...
class APLService:
    ...
    def gdf_merge_add_columns(self, gdf: pd.DataFrame) -> pd.DataFrame:
        ...
        gdf["apl_max"] = gdf[gdf["year"] == self.first_year]["apl"].max()
        gdf["apl_min"] = gdf[gdf["year"] == self.first_year]["apl"].min()
        gdf["apl_mean"] = gdf[gdf["year"] == self.first_year]["apl"].mean()
        return gdf
    ...
    def get_export(self, code: str, studies_df: pd.DataFrame, gdf: pd.DataFrame) -> tuple[dict, any]:
        ...
        cols = ['code_insee', 'nom_commune', 'lon', 'lat', 'fid', 'year', 'nb', 'apl', 'swpop', 'pop', 'pop_ajustee',
                'apl_max', "apl_min", "apl_mean", 'code_iris', 'nom_iris', "geometry"]
        if "tp60" in gdf:
    ...
    def compute_iris_csv(self,
        ...
        return gdf_merged[['code_insee', 'nom_commune', 'code_iris', 'nom_iris', 'year', 'nb', 'apl', 'pop', 'tp60',
    ...
    def compute_commune_csv(self,
        ...
        gdf_commune = self.group_apl_by_commune(gdf_merged)
        gdf_commune = self.gdf_merge_add_columns(gdf_commune)
        return gdf_commune[['code_insee', 'nom_commune', 'year', 'nb', 'apl', 'pop', 'tp60',
                           'med', 'gi', 'pop65p', 'pop65p_ratio_france']]
