...
class SanteSpecialiteParser:
    ...
    def parse(self):
        ...
        for index, row in self.dataframe.iterrows():
            ...
            e.v[18] = int(np.round(row[24])) if str(row[24]) != "nan" else ""
            e.v[19] = int(np.round(row[22])) if str(row[22]) != "nan" else ""
            e.v[20] = int(np.round(row[23])) if str(row[23]) != "nan" else ""
    ...
    def save(self):
        ...
        index = source.rindex(".")
        source = source[:index]
        file += f"ps-tarifs-{source}-12-06.csv"
...
if __name__ == '__main__':
    ...
    # data/SanteSpecialite/Santé_Spécialité_2_Pédiatres_201306_v0-97.xlsx
    # data/SanteSpecialite/Santé_Spécialité_1_Ophtalmologues_201306_v0-97.xlsx
    # data/SanteSpecialite/Santé_Spécialité_1_Gynécologues_201306_v0-97.xlsx