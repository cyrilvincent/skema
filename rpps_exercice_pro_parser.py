...
class RPPSExerciceProParser(BaseParser):
    ...
    def load_cache(self):
        ...
        l: List[Personne] = self.context.session.query(Personne).options(joinedload(Personne.exercice_pros)).all()
        for p in l:
    ...
    def mapper(self, row) -> ExercicePro:
        ...
        try:
            ...
            e.nom = row["Nom d'exercice"]
            e.prenom = row["Prénom d'exercice"]
    ...
    def make_relations(self, e: ExercicePro):
        try:
            e.personnes.append(self.personnes[e.inpp])
