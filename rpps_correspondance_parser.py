...
class RPPSCorrespondanceParser(BaseParser):
    ...
    def load_cache(self):
        print("Making cache")
        l = self.context.session.query(Diplome)
        for d in l:
    ...
    def mapper(self, row) -> Tuple[Optional[int], Optional[str], Optional[int], Optional[str]]:
        profession_id: Optional[int] = None
        savoir_faire_code: Optional[str] = None
        code_profession: Optional[int] = None
        diplome_rpps_code: Optional[str] = None
        try:
            ...
            diplome_rpps_code = self.get_nullable(row["diplome_rpps"])
        except Exception as ex:
            print(f"ERROR Correspondance row {self.row_num} {profession_id} {savoir_faire_code}\n{ex}\n{row}")
            quit(1)
        return profession_id, savoir_faire_code, code_profession, diplome_rpps_code

    def parse_row(self, row):
        id, code, cp, diplome_code = self.mapper(row)
        if id is not None:
            p: Profession = self.entities[id]
            update = False
            if code is not None:
                if code in self.diplomes:
                    s: Diplome = self.diplomes[code]
                    if s not in p.diplomes:
                        update = True
                        p.diplomes.append(s)
                else:
                    print(f"Warning row {self.row_num} savoir_faire {code} does not exist in diplome")
            if diplome_code is not None:
                if diplome_code in self.diplomes:
                    d: Diplome = self.diplomes[diplome_code]
                    if d not in p.diplomes:
                        update = True
                        p.diplomes.append(d)
                else:
                    print(f"Warning row {self.row_num} diplome_rpps {diplome_code} does not exist in diplome")
            if cp is not None:
...
if __name__ == '__main__':
    ...
    psp = RPPSCorrespondanceParser(context)
