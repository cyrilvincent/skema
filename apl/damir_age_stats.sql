...
create table apl.damir_age_stats_3 as(
with total_count as (
		select sum(prs_act_qte) total_nb, flx_ann_moi/100 ann, psp_spe_snds from damir
		where prs_rem_typ=0
		and prs_act_qte > 0
		group by ann, psp_spe_snds
	), total_count_2 as (
		select sum(prs_act_qte) total_nb, flx_ann_moi/100 ann, psp_act_snds from damir
		where prs_rem_typ=0
		and prs_act_qte > 0
		group by ann, psp_act_snds
	)
(select d.psp_spe_snds, null psp_act_snds, d.flx_ann_moi/100 ann, d.age_ben_snds, sum(prs_act_qte) nb, tc.total_nb, sum(prs_act_qte)/tc.total_nb::float ratio
from damir d
join total_count tc on tc.ann=d.flx_ann_moi/100 and tc.psp_spe_snds=d.psp_spe_snds
where prs_rem_typ=0
and prs_act_qte > 0
...
select null psp_spe_snds, d.psp_act_snds, d.flx_ann_moi/100 ann, d.age_ben_snds, sum(prs_act_qte), tc.total_nb, sum(prs_act_qte)/tc.total_nb::float ratio
from damir d
join total_count_2 tc on tc.ann=d.flx_ann_moi/100 and tc.psp_act_snds=d.psp_act_snds
where prs_rem_typ=0
and prs_act_qte > 0
