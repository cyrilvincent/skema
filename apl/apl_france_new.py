...
left join adresse_norm an on an.id=pands.adresse_norm_id -- TODO adresse_norm_id should be not null after remove H (drop_na) 
left join iris.iris i on i.code=an.iris -- TODO iris should be not null after remove H (drop_na) 
where sp.specialite_id={specialite}
and ds.annee={year}
--and pands.adresse_norm_id is not null
and (pands.adresse_norm_id is not null or ps.code = 'H')
...
for with_s in [False]:  # [True, False]
                ...
                for year in range(20, 27):  # 3*7*22*3=2772
                    ...
                    for specialite in [10]:  # range(1, 29):
                        ...
                        for accessibilite_exp in [-0.12]:  # [-0.12, -0.10, -0.08, -0.06, -0.04]:
                            ...
                            ps_df = ps_df.dropna(subset=["code"])
                            nb_ps = ps_df["id"].nunique()
                            ps_df["lon2"] = ps_df["lon"].round(2)
                            ps_df["lat2"] = ps_df["lat"].round(2)
                            nb_cabinet_ps = ps_df.groupby(["id", "lon2", "lat2"])
                            ...
                            ps_df["nbh"] = 0
                            ps_df.loc[ps_df["code"] == "H", "nbh"] = 1
                            ps_df["nbl"] = 0
                            ps_df.loc[ps_df["code"] == "L", "nbl"] = 1
                            ps_df["nbm"] = 0
                            ps_df.loc[ps_df["code"] == "M", "nbm"] = 1
                            ps_df["nbs"] = 0
                            ps_df.loc[(ps_df["code"] == "S") | (ps_df["code"] == "B"), "nbm"] = 1
                            ps_df["nbhg"] = ps_df.groupby(["id"])["nbh"].transform("sum")
                            ps_df["nblg"] = ps_df.groupby(["id"])["nbl"].transform("sum")
                            ps_df["nbmg"] = ps_df.groupby(["id"])["nbm"].transform("sum")
                            ps_df["nbsg"] = ps_df.groupby(["id"])["nbs"].transform("sum")
                            ps_df["nblg"].unique(), ps_df["nbhg"].unique(), ps_df["nbmg"].unique(), ps_df[
                                "nbs"].unique()

                            ps_df["lmh_sort"] = 4
                            ps_df.loc[ps_df["code"] == "L", "lmh_sort"] = 0
                            ps_df.loc[ps_df["code"] == "M", "lmh_sort"] = 1
                            ps_df.loc[ps_df["code"] == "H", "lmh_sort"] = 2
                            ps_df = ps_df.sort_values(by='lmh_sort')
                            ps_df

                            if with_s:

                                ps_df["hlm_weight"] = ps_df["nblg"] + ps_df["nbmg"] + ps_df["nbsg"] + 0.25 * ps_df[
                                    "nbhg"]
                                ps_df.loc[(ps_df["nblg"] > 0) & ((ps_df["nbmg"] > 0) | (ps_df["nbhg"] > 0)) & (
                                        ps_df["code"] == "L"), "hlm_weight"] = ps_df["nblg"] + ps_df["nbsg"] + 0.25 * (
                                        ps_df["nbmg"] + ps_df["nbhg"])
                                ps_df.loc[(ps_df["nblg"] > 0) & ((ps_df["nbmg"] > 0) | (ps_df["nbhg"] > 0)) & (
                                        (ps_df["code"] == "H") | (ps_df["code"] == "M")), "hlm_weight"] = 4 * (
                                        ps_df["nblg"] + ps_df["nbsg"] + 0.25 * (ps_df["nbmg"] + ps_df["nbhg"]))
                                ps_df.loc[ps_df["hlm_weight"] < 1, "hlm_weight"] = 1
                                ps_df["hlm_weight"] = ps_df["nblg"] + ps_df["nbmg"]
                                # ps_df["hlm_weight"].unique()

                                ps_df = ps_df[(ps_df["code"] == "L") | (ps_df["code"] == "M")]
                            else:
                                # ps_df["hlm_weight"] = ps_df["nblg"] + 0.25 * ps_df["nbhg"]
                                # ps_df.loc[(ps_df["nblg"] > 0) & (ps_df["nbhg"] > 0) & (
                                #         ps_df["code"] == "L"), "hlm_weight"] = ps_df["nblg"] + 0.25 * ps_df["nbhg"]
                                # ps_df.loc[(ps_df["nblg"] > 0) & (ps_df["nbhg"] > 0) & (
                                #         (ps_df["code"] == "H")), "hlm_weight"] = 4 * (ps_df["nblg"] + 0.25 * ps_df["nbhg"])
                                # ps_df.loc[ps_df["hlm_weight"] < 1, "hlm_weight"] = 1

                                #ps_df["hlm_weight"] = ps_df["nblg"] + 0.25 * ps_df["nbhg"]
                                ps_df["hlm_weight"] = ps_df["nb_cabinet"]
                                ps_df["hlm_weight"].unique()

                                ps_df = ps_df[ps_df["code"] == "L"]

                            # ps_df = ps_df.dropna(subset=["adresse_norm_id", "iris"])

                            # ps_df["weight"] = (1 / ps_df["nb_cabinet"]).replace(np.inf, 0)
                            ps_df["weight"] = (1 / ps_df["hlm_weight"]).replace(np.inf, 0)

                            ps_df["nb"] = ps_df.groupby("iris")["weight"].transform("sum")

                            ps_df["nb"] = ps_df["nb"].fillna(0)
                            ps_df = ps_df.sort_values(by='lmh_sort')
                            ps_df = ps_df.dropna(subset=["iris", "adresse_norm_id"])
                            ps_df = ps_df.drop_duplicates(subset=["iris", "id"])
